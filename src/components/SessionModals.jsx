...
export const SessionReportModal = ({ isOpen, data, onClose, onRestart }) => {
  ...
  const handleShare = async () => {
    setShareStatus("⏳ Generating Badge...");
    ...
    try {
      ...
      grad.addColorStop(1, '#1a1c2e');
      ctx.fillStyle = grad;
      ctx.fillRect(0, 0, 1200, 630);

      // 2. Artistic Blobs (Focus Energy)
      ctx.globalAlpha = 0.4;
      ...
      ctx.fillText('🛡️ DEEPWORK GUARD SYSTEM', 60, 80);
      
      ctx.fillStyle = '#ffffff';
      ctx.font = 'bold 80px Outfit, sans-serif';
      ctx.fillText('Session Report', 60, 170);

      // 4. Task Name
      ctx.fillStyle = 'rgba(255,255,255,0.5)';
      ctx.font = '30px Outfit, sans-serif';
      ctx.fillText(`Task: ${taskName}`, 65, 230);

      // 5. Focus Score (The Hero)
      const scoreColor = focusScore >= 80 ? '#10b981' : focusScore >= 60 ? '#f59e0b' : '#ef4444';
      
      // Draw Score Pill
      ctx.fillStyle = 'rgba(255,255,255,0.03)';
      ctx.strokeStyle = 'rgba(255,255,255,0.1)';
      ctx.beginPath();
      ctx.roundRect(850, 80, 280, 280, 40);
      ctx.fill();
      ctx.stroke();
      ...
      ctx.fillText(`${focusScore}%`, 990, 230);
      
      ctx.fillStyle = '#ffffff';
      ctx.font = 'bold 24px Outfit, sans-serif';
      ctx.fillText('FOCUS SCORE', 990, 280);
      ctx.textAlign = 'left';
      ...
      drawStat('DURATION', formatTime(data.duration || focusedTime), 65, 380);
      drawStat('DISTRACTIONS', `${totalDistractions} ALERTS`, 400, 380);
      drawStat('PEAK STREAK', formatTime(longestStreak), 735, 380);

      // 7. Footer / Branding
      ctx.fillStyle = 'rgba(255,255,255,0.2)';
      ctx.font = '20px Outfit, sans-serif';
      ctx.fillText('Generated by DeepWork Guard OS • Certified Focus Session', 60, 570);

      // 8. Seal / Badge
      ctx.strokeStyle = scoreColor;
      ctx.lineWidth = 4;
      ctx.beginPath();
      ctx.arc(1100, 530, 50, 0, Math.PI * 2);
      ctx.stroke();
      ctx.fillStyle = scoreColor;
      ctx.textAlign = 'center';
      ctx.font = 'bold 16px Outfit, sans-serif';
      ctx.fillText('PASSED', 1100, 535);
      ...
      setShareStatus("✅ Badge Downloaded!");
      setTimeout(() => setShareStatus("📤 Share Results"), 3000);
    } catch (err) {
      console.error(err);
      setShareStatus("❌ Engine Error");
    }
  };
